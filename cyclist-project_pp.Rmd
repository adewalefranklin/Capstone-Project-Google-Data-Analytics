---
title: "Cyclistic_BIke_Project"
author: "Adewale Franklin Hassan"
date: "`r Sys.Date()`"
output:
...
```
```{r}
#STEP 3 - Getting to know the data: head() for viewing the first 6 rows of the table, Colnames() to check column names, glimpse() to check the characteristics of the dataframe
 
# STEP 4- RENAMING THE COLUMNS CONSISTENCY
...
```
```{r}
#STEP 5 - View the columns to see the results of the merging
...
```
```{r}
#STEP 13 - Final cleaning (DROP COLUMNS NOT REQUIRED FOR VISUALISATION)

# STEP 14 - RENAME DATAFRAME
...
```
```{r}
# 2. - AVERAGE DAILY RIDE LENGTH BETWEEN CASUAL RIDERS AND MEMBERSHIP RIDERS
...
```
```{r}
# 3. AVERAGE DAILY RIDE LENGTH BASED ON BIKE TYPE AND MEMBERSHIP TYPE
...
cyclistic_cleaned_data %>% 
  ...
  labs(title = "Sum of Rides by Days of The week & Membership Types",x='Days of the Week',y='Sum of Rides')+
  theme_classic()
```
```{r}
